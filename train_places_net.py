...
def minivggnet(data, labels=None, train=False, cudnn=False, param=learned_param,
    ...
    n.data = data
    conv_kwargs = dict(param=param, train=train, cudnn=cudnn)
    n.conv1, n.relu1 = conv_relu(n.data, 25, 54, stride=2, **conv_kwargs)
    n.norm1 = L.LRN(n.relu1, local_size=5, alpha=0.0005, beta=0.75, k=2)
    n.pool1 = max_pool(n.norm1, 3, stride=2, train=train, cudnn=cudnn)
    n.conv2, n.relu2 = conv_relu(n.pool1, 5, 128, pad=1, stride=2, group=2, **conv_kwargs)
    n.norm2 = L.LRN(n.relu2, local_size=5, alpha=0.0005, beta=0.75, k=2)
    n.pool2 = max_pool(n.norm2, 3, stride=2, train=train, cudnn=cudnn)
    n.conv3, n.relu3 = conv_relu(n.pool2, 3, 128, pad=1, **conv_kwargs)
    n.conv4, n.relu4 = conv_relu(n.relu3, 3, 128, pad=1, group=2, **conv_kwargs)
    n.conv5, n.relu5 = conv_relu(n.relu4, 3, 128, pad=1, group=2, **conv_kwargs)
    n.pool5 = max_pool(n.relu5, 3, stride=2, train=train, cudnn=cudnn)
    n.fc6, n.relu6 = fc_relu(n.pool5, 512, param=param)
    n.drop6 = L.Dropout(n.relu6, in_place=True)
    n.fc7, n.relu7 = fc_relu(n.drop6, 128, param=param)
    n.drop7 = L.Dropout(n.relu7, in_place=True)   
    preds = n.fc8 = L.InnerProduct(n.drop7, num_output=num_classes, param=param)
    if not train:
        # Compute the per-label probabilities at test/inference time.
...
def train_net(args, with_val_net=False):
    ...
    solver.net.params['conv1'][0].data[...] = weights.make_lm_filters()
